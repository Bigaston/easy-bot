...
module.exports = {
	...
	event : function triggerEvent(pText, client) {
		...
		if (pText.match(/!channel:setname\[[0-9]*\]:\[[-0-9a-z]*\]!/) != null) {
			trig = pText.match(/!channel:setname\[[0-9]*\]:\[[-0-9a-z]*\]!/g);

			for (i = 0; i < trig.length; i++) {
				chan = client.channels.get(trig[i].replace("!channel:setname[", "").replace(/]:\[[-0-9a-z]*\]!/, ""))
				chan.edit({ name: trig[i].replace(/!channel:setname\[[0-9]*\]:\[/, "").replace("]!", "")})
