...
function replaceGlobalArg(pText, pServ) {
	...
	var h = new Date();
	text = text.replace("%date%", sendLang("dateFormat").replace("dd", addZero(h.getDay())).replace("mm", addZero(h.getMonth())).replace("yyyy", h.getFullYear()));
	text = text.replace("%day%", h.getDay());
